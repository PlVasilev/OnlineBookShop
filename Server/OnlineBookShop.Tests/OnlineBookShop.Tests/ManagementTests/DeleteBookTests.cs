...
namespace OnlineBookShop.Tests.ManagementTests
{
    class DeleteBookTests
    {
       ...
       "author", 100, 10, 1000, 20, 40, 5, TestName = "Delete_Book_Whit_Valid_Id")]
       ...
       int numberOfPurchases)
        {
            ...
            request = new RestRequest(BaseURL + $"/Management/DeleteBook/{bookId}", Method.DELETE);
            request.AddHeader("Authorization", string.Format("Bearer {0}", _token));
        ...
        [TestCase("title", "description", "summary", "1234567891011",
            "https://images.unsplash.com/photo-1598618443855-232ee0f819f6?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=667&q=80",
            "author", 100, 10, 1000, 20, 40, 5, TestName = "Delete_Book_Whit_Invalid_Id")]
        public void Delete_Book_Whit_Invalid_Id(
            string title,
            string description,
            string summaryDescription,
            string isbn,
            ...
            int numberOfPurchases)
        {
            ...
            request = new RestRequest(BaseURL + $"/Management/DeleteBook/{Guid.NewGuid().ToString()}", Method.DELETE);
            request.AddHeader("Authorization", string.Format("Bearer {0}", _token));
            response = _client.Execute(request);
